...
public interface EmployeeRepository extends JpaRepository<Employee, String> {
	...
	@Query(nativeQuery = true, value="select t1.emp_id,t1.employee_id,t1.sur_name,t1.given_name, " +
			...
			" from sf_emp_employee t1 INNER JOIN sf_com_emp_grp t2 on t1.emp_id = t2.emp_id " +
			" where t2.comp_id = ?1 ")
	...
	@Query(nativeQuery = true, value="select t1.emp_id,t1.employee_id,t1.sur_name,t1.given_name, " +
			...
			countQuery = "select count(*) from sf_emp_employee t1 INNER JOIN sf_com_emp_grp t2 on t1.emp_id = t2.emp_id " +
			" where t2.comp_id = ?1 ")
	Page<Employee> findPageByCompId(String compId, Pageable pageable);
	
	@Query(nativeQuery = true, value="select t1.emp_id from sf_emp_employee t1 INNER JOIN sf_com_emp_grp t2 on t1.emp_id = t2.emp_id " +
			" where t2.comp_id = ?1 and t1.employee_id = ?2 ")
	String getEmpIdBy(String compId, String employeeId);
	
	@Query(nativeQuery = true, value="delete from sf_emp_employee where emp_id in ( "
